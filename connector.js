...
class ServiceNowConnector {
  ...
  processRequestResults(error, response, body, callback) {
    ...
    if (error) {
       console.log("\nprocessRequestResults: Error %s",error)
       callbackError = error;
       return callback( callbackData, callbackError);
    }
    if (this.isHibernating(response))
       return callback( null, "Service Now Instance hibernating");

    if ((response.statusCode == 200) || (response.statusCode == 201)) {
       console.log("\nprocessRequestResults: Valid Return Code") 
       return callback(callbackData, callbackError);
    }
    else {
       callbackError = "Bad HTTP return code";
       return callback(callbackData, callbackError);
    }
  }
  ...
  sendRequest(callOptions, callback) {
    ...
    console.log("\nURI: " + requestOptions.baseUrl + uri );
